...
class ManjaroUpdater:
    ...
    def check_dependencies(self):
        ...
        def run():
            ...
            try:
                commands = [
                    ("sudo pacman -Qk", "Проверка зависимостей пакетов"),
                ]
                ...
                if success and self.running:
                    ...
                    self.update_status("✓ Проверка зависимостей завершена", "green")
                elif not self.running:
                    self.append_output("\n⚠ Проверка зависимостей отменена\n")
                    self.update_status("⚠ Отменено", "orange")
    ...
    def fix_dependencies(self):
        ...
        def run():
            self.start_progress()
            try:
                # Сначала проверяем, потом исправляем
                self.append_output("\n--- Проверка проблемных пакетов ---\n")
                
                # Проверка сломанных пакетов
                check_cmd = "sudo pacman -Qkk 2>/dev/null | grep -E '(missing|changed)' || echo 'Нет проблем'"
                if not self.run_command(check_cmd, "Проверка сломанных пакетов"):
                    self.append_output("Предупреждение: Некоторые проверки могут быть неудачными\n")
                
                # Исправление зависимостей
                commands = [
                    ("sudo pacman -Syu --noconfirm", "Обновление системы"),
                    ("sudo pacman -D --asdeps $(pacman -Qtdq) 2>/dev/null || echo 'Нет остаточных пакетов'", "Отметка остаточных пакетов как зависимых"),
                    ("sudo pacman -S --noconfirm $(pacman -Qqd) 2>/dev/null || echo 'Нет пакетов для переустановки'", "Переустановка подозрительных пакетов"),
                ]
                ...
                if success and self.running:
                    ...
                    self.update_status("✓ Исправление зависимостей завершено", "green")
                elif not self.running:
                    self.append_output("\n⚠ Исправление зависимостей отменено\n")
                    self.update_status("⚠ Отменено", "orange")
    ...
    def clean_orphans(self):
        ...
        def run():
            ...
            try:
                commands = [
                    ("sudo pacman -Rns $(pacman -Qtdq) --noconfirm 2>/dev/null || echo 'Нет остаточных пакетов для удаления'", "Удаление остаточных пакетов"),
                ]
                ...
                if success and self.running:
                    self.append_output("\n✓ Удаление остаточных пакетов завершено!\n")
                    self.update_status("✓ Удаление остаточных пакетов завершено", "green")
                elif not self.running:
                    self.append_output("\n⚠ Удаление остаточных пакетов отменено\n")
                    self.update_status("⚠ Отменено", "orange")
        ...
        def run():
            ...
            try:
                commands = [
                    ...
                    ("sudo rm -f /var/log/*.log.* 2>/dev/null || echo 'Нет старых логов для удаления'", "Удаление старых файлов логов"),
                ]
    ...
    def full_clean(self):
        ...
        def run():
            ...
            try:
                self.append_output("\n=== ПОЛНАЯ ОЧИСТКА СИСТЕМЫ ===\n")
                self.append_output("Это очистит: кэш пакетов, остаточные пакеты, логи и кэш yay\n")
                
                commands = [
                    ("sudo pacman -Sc --noconfirm", "Очистка кэша пакетов"),
                    ("sudo pacman -Rns $(pacman -Qtdq) --noconfirm 2>/dev/null || echo 'Нет остаточных пакетов'", "Удаление остаточных пакетов"),
                    ("yay -Sc --noconfirm", "Очистка кэша yay"),
                    ("sudo journalctl --vacuum-time=7d", "Очистка systemd журналов"),
                    ("sudo pacman -Scc --noconfirm", "Очистка всего кэша пакетов (опционально)"),
                ]
